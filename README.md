...
└─────────────────────────────────────────────────────────────────┘
  ...
  用户删除：仅删除本地副本，不再请求远程删除
    ...
    │  弹窗内部表面同步玻璃化（背板在所有设备都有模糊，故无条件应用，仅弹窗内）：TextInput/选择行用 color_bg_input_glass（85% 加深灰蓝，亮 #D9E4E8EE/暗 #D9333E4B——浅灰 70% 叠在白玻璃上与背板明度差太小看不出是输入槽，故加深一档做内凹层次）、次级按钮（取消等）为透明幽灵按钮（Color.Transparent 无底，60% 玻璃灰叠白背板会显发白方块故弃用 color_bg_control_glass）、内嵌卡片/服务器行用 color_bg_surface_glass、选中态用 color_bg_selected_glass（20% 品牌橙，含算法/模式 Menu 选中行）；主 CTA（添加/保存）保持实色品牌橙不变
